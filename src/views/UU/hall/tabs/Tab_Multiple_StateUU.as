package views.UU.hall.tabs 
{
	...
	//竞技模式
	public class Tab_Multiple_StateUU extends StateUU
	{
		...
		override public function onEnter():void 
		{
			...
			img.source = TResourceManager.getAsset("hall/img/four_B.png");
			...
			btn.locatingOffsetY = 3;
			this.fusion.addNode(btn);
			btn.addEventListener(ATouchEvent.CLICK, onEightClick);
			btn.touchable = false;
			
			img = new BitmapLoaderUU;
			img.source =  btn.touchable ? TResourceManager.getAsset("hall/img/eight_B.png") : TResourceManager.getAsset("hall/img/eight_A.png");
			img.locatingTypeForHoriz = LocatingType.F___A___F;
			img.locatingTypeForVerti = LocatingType.F___A___F;
			//label = TextUtil.createLabel("进入模式", 33, 0xdddd33, true, 6);
			btn.addNode(img);
			...
			// Role.
			img = new BitmapLoaderUU;
			img.source = TResourceManager.getAsset("hall/img/multiple_role.png");
			this.fusion.addNode(img);
			//img.x = 1100;
			img.locatingTypeForHoriz = LocatingType.F____A_F
			img.locatingOffsetX = 10;
			img.y = 30;
			img.touchable = false;
			
						
			//模式描述
			dscLabel = TextUtil.createLabel5(HallConfig.MULTI_DSC_TEXT, 24, ColorUtil.WHITE, false,2,3.5,22,"黑体");
			dscLabel.width = 500;
			dscLabel.height = 200;
		...
		private function onTwoClick(e:ATouchEvent):void {
			GameModel.gameMode = RoomMod.ROOM_MOD_ARENA_1V1;
			onSelectMode(e);
		}
		private function onFourClick(e:ATouchEvent):void {
			GameModel.gameMode = RoomMod.ROOM_MOD_ARENA_2V2;
			onSelectMode(e);
		}
		private function onFiveClick(e:ATouchEvent):void {
			GameModel.gameMode = RoomMod.ROOM_MOD_ARENA_2V3;
			onSelectMode(e);
		}
		private function onSixClick(e:ATouchEvent):void {
			GameModel.gameMode = RoomMod.ROOM_MOD_ARENA_3V3;
			onSelectMode(e);
		}
		private function onEightClick(e:ATouchEvent):void {
			GameModel.gameMode = RoomMod.ROOM_MOD_ARENA_4V4;
			onSelectMode(e);
		}
